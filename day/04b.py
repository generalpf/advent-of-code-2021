...
while len(calls) > 0:
    ...
    non_winning_cards = [card for card in cards if not did_card_win(card)]
    if len(cards) == 1 and len(non_winning_cards) == 0:
        score = card_score(cards[0], call)
        print(f"score = {score}")
        exit(0)
    cards = non_winning_cards
